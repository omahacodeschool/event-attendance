class Event
  ...
  def Event.create(params)
    values = [$database.next_id("events"),params[:group_name],params[:title],params[:date],Time.parse(params[:time]).strftime("%I:%M %p"),params[:location],params[:address],params[:link]]
